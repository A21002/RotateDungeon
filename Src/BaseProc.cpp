...
//------------------------------------------------------------------------
BOOL CBaseObj::TargetMove( float m_nSpeed)
{
	float mvX, mvY, n;
	BOOL ret = FALSE;
	...
	if (m_vPos.x == m_vTarget.x && m_vPos.y == m_vTarget.y) {  // 目的地に達したとき
		...
		if (abs(mvX) >= abs(mvY)) {   // Ｘ方向の距離が長いとき
			if (abs(mvX) < m_nSpeed) {        // １回の移動量より近接しているとき
				m_vPosUp.x = mvX;
				m_vPosUp.y = mvY;
			}
			else {
				if (mvX >= 0) {
					m_vPosUp.x = m_nSpeed; // Ｘ方向の移動量をm_nSpeedにする
				}
				else {
					m_vPosUp.x = -m_nSpeed; // Ｘ方向の移動量を-m_nSpeedにする
				}
				n = abs(mvX / m_nSpeed);
			...
			if (abs(mvY) < m_nSpeed) {        // １回の移動量より近接していると
				m_vPosUp.x = mvX;
				m_vPosUp.y = mvY;
			}
			else {
				if (mvY >= 0) {
					m_vPosUp.y = m_nSpeed; // Ｙ方向の移動量をm_nSpeedにする
				}
				else {
					m_vPosUp.y = -m_nSpeed; // Ｙ方向の移動量を-m_nSpeedにする
				}
				n = abs(mvY / m_nSpeed);
